        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form method="post" action="{{ route('memos.destroy',$memos[$i]->id)}}">
                    @csrf
                    @method('DELETE')
                    <button class="text-white bg-neutral-300 border-0 px-1 focus:outline-none hover:bg-neutral-400 rounded block float-left my-10 opacity-0 delete">×</button>
                </form>
                    <div class="p-6 bg-white border-b border-gray-200 memos">
                        ...
                        @for ($ti = 0; $ti < count($tags[$i]); $ti++)
                            <span class="text-sm">#{{$tags[$i][$ti]['name']}}</span>
                        @endfor
                    </div>
                @endfor
            </div>
